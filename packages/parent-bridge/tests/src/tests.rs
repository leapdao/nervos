...
fn test_deposit_with_lock() {
    ...
    let deposit_lock_args : Bytes = Default::default();
    let depsit_lock_bin: Bytes = Loader::default().load_binary("deposit-lock");
    let depsit_lock_out_point = context.deploy_cell(depsit_lock_bin);
    let depsit_lock_script = context
        .build_script(&depsit_lock_out_point, deposit_lock_args)
        .expect("script");
    let depsit_lock_dep = CellDep::new_builder().out_point(depsit_lock_out_point).build();
